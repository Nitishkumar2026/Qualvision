...
export function PartsList({
  ...
  const unknownCount = parts.filter(p => p.status === 'unknown').length;
  ...
  return (
    <div className="flex flex-col h-full bg-card border-l md:border-border border-transparent w-full md:w-80">
        ...
        <div className="flex gap-2 text-xs text-muted-foreground mb-4">
          {mode === 'dimensional' ? (
            ...
            <>
              ...
              <span>•</span>
              <span className="text-yellow-500">{unknownCount} Unlabeled</span>
            </>
          )}
        </div>
        ...
        <div className="divide-y divide-border/50">
          {parts.map((part) => (
            <div
              key={part.id}
              className={cn(
                "p-3 transition-all group cursor-pointer border-l-2",
            ...
            >
              <div className="flex items-start justify-between mb-1">
                <span className="font-mono text-xs text-muted-foreground group-hover:text-primary transition-colors">#{part.id}</span>
                {mode === 'dimensional' ? (
              ...
              <div className="font-medium text-sm truncate group-hover:text-primary transition-colors" title={part.name}>
                {part.name}
              </div>
      ...
      <div className="p-4 border-t border-border bg-muted/10">
        <div className="text-[10px] text-center text-muted-foreground">
          {mode === 'dimensional' ? 'Analysis completed using ORB + SSIM matching' : 'Analysis completed in 124ms'}
        </div>
      </div>
    </div>
  );
